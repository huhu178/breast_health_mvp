...
class Config:
    ...
    OPENROUTER_MODEL = os.getenv('OPENROUTER_MODEL', 'qwen/qwen3.6-plus')  # 默认使用gemini 2.5 pro
    ...
    # ========================================
    # AI舌诊开放平台（企业应用）配置：服务端代调用
    # ========================================
    # @isdoc 开放平台 base url（默认官方域名）
    AI_TONGUE_BASE_URL = os.getenv('AI_TONGUE_BASE_URL', 'https://www.ai-tongue.com')
    # @isdoc 企业应用 devId / devSecret（敏感信息，仅服务端使用）
    AI_TONGUE_DEVID = os.getenv('AI_TONGUE_DEVID', '')
    AI_TONGUE_DEVSECRET = os.getenv('AI_TONGUE_DEVSECRET', '')

    # @isdoc 可选：限制小程序调用你们后端的共享 token（header: X-Miniprogram-Token）
    MINIPROGRAM_API_TOKEN = os.getenv('MINIPROGRAM_API_TOKEN', '')
