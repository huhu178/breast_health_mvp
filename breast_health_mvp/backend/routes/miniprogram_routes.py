...
def submit_questionnaire():
    ...
    try:
        ...
        # 从 normalized_data 和 c_patient、c_record 构建 template_fields
        # 创建一个临时对象来模拟 BPatient/BHealthRecord 的接口，用于 extract_template_fields
        class TempPatient:
            def __init__(self, c_patient, data):
                self.name = c_patient.name
                self.gender = data.get('gender', '女')
                self.phone = c_patient.phone
                self.age = clean_numeric_value(data.get('age'), 'int')
        
        class TempRecord:
            """临时记录对象，用于兼容 extract_template_fields 函数"""
            def __init__(self, c_record, data):
                # 从 c_record 或 data 中获取值
                self._data = data
                self._c_record = c_record
                # 基本字段（直接设置，避免 __getattr__ 调用）
                self.age = c_record.age or clean_numeric_value(data.get('age'), 'int')
                self.height = clean_numeric_value(data.get('height'), 'float')
                self.weight = clean_numeric_value(data.get('weight'), 'float')
                self.phone = c_record.patient.phone if hasattr(c_record, 'patient') and c_record.patient else None
                # 日期字段需要特殊处理（parse_date 返回的是 date 对象）
                ...
                # 其他字段通过 __getattr__ 动态获取
                
            def __getattr__(self, name):
                """动态获取字段值，优先从 data 获取，其次从 c_record 获取"""
                # 先从 data 中获取
                if name in self._data:
                    ...
                    if value is not None:
                        return value
                # 再从 c_record 中获取
                if hasattr(self._c_record, name):
                    return getattr(self._c_record, name)
                # 返回 None（extract_breast_fields 等函数会处理 None 值）
                return None
        
        temp_patient = TempPatient(c_patient, data)
        temp_record = TempRecord(c_record, data)
        ...
        # 7. 保存C端报告（只保留C端数据，不再创建B端影分身）
        c_report = CReport(
            patient_id=c_patient.id,
            record_id=c_record.id,
            report_code=f"CPR{now.strftime('%Y%m%d%H%M%S')}{uuid.uuid4().hex[:4].upper()}",
            report_html=report_html,
            imaging_conclusion=conclusion,  # 保存影像学评估结论（用于B端审核）
            imaging_risk_warning=risk_warning,  # 保存风险提示（用于B端审核）
        ...
        return Response.success({
            ...
            'report_code': c_report.report_code,
            'report_html': report_html,  # 小程序可直接展示
            'patient_code': c_patient.patient_code,
            'openid': openid,
            'need_manager_contact': need_manager_contact,  # 是否需要添加健康管理师联系方式
            'patient_id': c_patient.id  # 用于后续提交管理师联系方式
        }, '问卷提交成功，报告已生成（C端已入库）')
