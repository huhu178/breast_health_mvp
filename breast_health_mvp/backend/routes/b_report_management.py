...
def finalize_report(current_user, report_id):
    ...
    try:
        ...
        print(f"   已批准数: {len(approved_recs)}")
        for i, rec in enumerate(recommendations):
            status = "✅" if rec.get('is_approved', False) else "❌"
            print(f"   {status} 建议 #{i}: {rec.get('category', 'Unknown')} - is_approved={rec.get('is_approved', False)}")
        
        if not approved_recs:
            return Response.error('没有已批准的建议，无法生成最终报告。请先在建议审核页面批准至少一条建议。', 400)
