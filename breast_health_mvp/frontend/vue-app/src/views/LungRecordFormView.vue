...
async function submitForm() {
  ...
  try {
    ...
    if (hasFiles) {
      ...
      formDataObj.append('nodule_types', 'lung')
      ...
      formDataObj.append('lung_symptoms', Array.isArray(formData.value.lung_symptoms)
      ...
      formDataObj.append('nodule_quantity', formData.value.nodule_quantity || '')
      formDataObj.append('lung_nodule_count', formData.value.lung_nodule_count || '')
      formDataObj.append('lung_nodule_size', formData.value.lung_nodule_size || '')
      ...
      formDataObj.append('family_history', Array.isArray(formData.value.family_history)
        ? formData.value.family_history.join(',')
        : (formData.value.family_history || ''))
      formDataObj.append('family_history_other', formData.value.family_history_other || '')
      formDataObj.append('medication_history', Array.isArray(formData.value.medication_history)
        ? formData.value.medication_history.join(',')
        : (formData.value.medication_history || ''))
      formDataObj.append('medication_other', formData.value.medication_other || '')
      ...
      const submitData = {
        ...
        family_history: Array.isArray(formData.value.family_history)
          ? formData.value.family_history.join(',')
          : formData.value.family_history,
        family_history_other: formData.value.family_history_other,
        medication_history: Array.isArray(formData.value.medication_history)
          ? formData.value.medication_history.join(',')
          : formData.value.medication_history,
        medication_other: formData.value.medication_other
      }
    ...
    <div class="form-layout">
      <form @submit.prevent="submitForm">
        ...
        <section class="form-section imaging-section">
          <div class="section-header">
            <div class="section-icon"></div>
            <h3 class="section-title">（二）肺部影像学特征</h3>
          </div>

          <!-- 使用 LungFormSection 组件 -->
          <LungFormSection v-model="formData" />
        </section>
