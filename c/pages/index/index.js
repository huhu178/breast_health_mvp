...
Page({
  ...
  verifyPhoneFormat(phone) {
    ...
    // 使用与后端相同的正则：^1[3-9]\d{9}$
    const phoneRegex = /^1[3-9]\d{9}$/
    if (!phoneRegex.test(phoneStr)) {
      if (phoneStr.length !== 11) {
        return { isValid: false, message: '手机号必须是11位数字' }
      }
      return { isValid: false, message: '手机号格式错误（请输入中国大陆手机号）' }
    }
  ...
  onPhoneInput(e) {
    ...
    // 实时验证手机号格式
    const validation = this.verifyPhoneFormat(phone)
    if (phone && phone.trim() && !validation.isValid) {
      // 如果已输入内容但格式不正确，立即提示
      wx.showToast({ 
  ...
  checkCanStart() {
    ...
    // 验证手机号格式（不仅检查长度，还要检查格式）
    const phoneValidation = this.verifyPhoneFormat(phone)
