...
// 表单数据 - 对齐 ThyroidFormSection（新表单字段体系）
const formData = ref({
  // （一）基本信息
  age: '',
  height: '',
  ...
  // （二）甲状腺影像学特征（与 ThyroidFormSection 一致）
  discovery_date: '',
  symptoms: [],
  symptoms_other: '',
  tirads_level: '',
...
async function submitForm() {
  ...
  try {
    ...
    if (hasFiles) {
      ...
      formDataObj.append('nodule_types', 'thyroid')

      // （一）基本信息
      // 注意：FormData.append() 会自动将值转换为字符串，需要确保数值字段正确转换
      // 对于数值字段，需要检查是否为 null/undefined/空字符串，但不能将 0 视为空值
      ...
      appendValue('age', formData.value.age)
      appendValue('height', formData.value.height)
      appendValue('weight', formData.value.weight)
      appendValue('phone', formData.value.phone)
      ...
      // （二）甲状腺影像学特征（新表单字段 → 后端入库字段）
      formDataObj.append('thyroid_discovery_date', formData.value.discovery_date || '')
      formDataObj.append('tirads_level', formData.value.tirads_level || '')
      formDataObj.append('thyroid_symptoms', Array.isArray(formData.value.symptoms)
      ...
      formDataObj.append('nodule_count', formData.value.nodule_count || '')
      // 甲状腺报告/模板使用 thyroid_nodule_* 字段：分别写入“数量(单发/多发)”与“多发个数”
      formDataObj.append('thyroid_nodule_quantity', formData.value.nodule_quantity || '')
      formDataObj.append('thyroid_nodule_count', formData.value.nodule_count || '')
      formDataObj.append('thyroid_nodule_size', formData.value.nodule_size || '')
      // 甲状腺基础疾病史/家族史/用药史
      ...
      // （二）甲状腺影像学特征（新表单字段 → 后端入库字段）
      thyroid_discovery_date: formData.value.discovery_date,
        tirads_level: formData.value.tirads_level,
      thyroid_symptoms: Array.isArray(formData.value.symptoms)
        ? formData.value.symptoms.join(',')
        : formData.value.symptoms,
      ...
      thyroid_nodule_quantity: formData.value.nodule_quantity,
      thyroid_nodule_count: formData.value.nodule_count,
      thyroid_nodule_size: formData.value.nodule_size,
  ...
  <div class="record-form-view">
    ...
    <header class="page-header-compact">
      <div class="header-left">
        ...
        <h2 class="page-title">甲状腺结节档案</h2>
      </div>
    ...
    <div class="form-layout">
      <form @submit.prevent="submitForm">
        ...
        <section class="form-section imaging-section">
          ...
          <ThyroidFormSection v-model="formData" />
        </section>
