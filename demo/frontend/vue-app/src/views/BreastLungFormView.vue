...
import LungFormSection from '../components/LungFormSection.vue'
...
function validateForm() {
  ...
  // 肺结节验证
  if (!formData.value.lung_rads_level) {
    alert('请选择Lung-RADS分级！')
    return false
  }
  if (!formData.value.lung_discovery_date) {
    alert('请填写肺结节发现时间！')
    return false
  }
...
async function submitForm() {
  ...
  try {
    ...
    if (hasFiles) {
      ...
      formDataObj.append('nodule_types', 'breast_lung')
      ...
      formDataObj.append('breast_symptoms_other', formData.value.symptoms_other || '')
      ...
      // （三）肺部影像学特征 - 使用带lung_前缀的字段名（部分已有）
      formDataObj.append('lung_discovery_date', formData.value.lung_discovery_date || '')
      formDataObj.append('symptoms_lung', Array.isArray(formData.value.lung_symptoms)
        ? formData.value.lung_symptoms.join(',')
        : (formData.value.lung_symptoms || ''))
      formDataObj.append('lung_symptoms_other', formData.value.lung_symptoms_other || '')
      formDataObj.append('lung_rads_level', formData.value.lung_rads_level || '')
      formDataObj.append('nodule_quantity_lung', formData.value.lung_nodule_quantity || '')
      formDataObj.append('nodule_count_lung', formData.value.lung_nodule_count || '')
      formDataObj.append('lung_size', formData.value.lung_nodule_size || '')
      formDataObj.append('lung_cancer_history', Array.isArray(formData.value.lung_cancer_history)
        ? formData.value.lung_cancer_history.join(',')
        : (formData.value.lung_cancer_history || ''))
      formDataObj.append('lung_cancer_history_other', formData.value.lung_cancer_history_other || '')
      // 肺部家族史/用药史（按器官）
      formDataObj.append('lung_family_history', Array.isArray(formData.value.lung_family_history)
        ? formData.value.lung_family_history.join(',')
        : (formData.value.lung_family_history || ''))
      formDataObj.append('lung_family_history_other', formData.value.lung_family_history_other || '')
      formDataObj.append('lung_medication_history', Array.isArray(formData.value.lung_medication_history)
        ? formData.value.lung_medication_history.join(',')
        : (formData.value.lung_medication_history || ''))
      formDataObj.append('lung_medication_other', formData.value.lung_medication_other || '')
      ...
      const submitData = {
        ...
        // （二）乳腺影像学特征
        // 兼容后端多器官字段：乳腺发现时间应写入 breast_discovery_date
        breast_discovery_date: formData.value.discovery_date,
        // 兼容旧字段（不依赖但保留，避免其它逻辑读取）
        discovery_date: formData.value.discovery_date,
        symptoms: Array.isArray(formData.value.symptoms)
          ? formData.value.symptoms.join(',')
          : formData.value.symptoms,
        breast_symptoms_other: formData.value.symptoms_other,
        ...
        // （三）肺部影像学特征
        lung_discovery_date: formData.value.lung_discovery_date,
        lung_symptoms: Array.isArray(formData.value.lung_symptoms)
          ? formData.value.lung_symptoms.join(',')
          : formData.value.lung_symptoms,
        lung_symptoms_other: formData.value.lung_symptoms_other,
        lung_rads_level: formData.value.lung_rads_level,
        lung_nodule_quantity: formData.value.lung_nodule_quantity,
        lung_nodule_count: formData.value.lung_nodule_count,
        lung_nodule_size: formData.value.lung_nodule_size,
        lung_cancer_history: Array.isArray(formData.value.lung_cancer_history)
          ? formData.value.lung_cancer_history.join(',')
          : formData.value.lung_cancer_history,
        lung_cancer_history_other: formData.value.lung_cancer_history_other,
        lung_family_history: Array.isArray(formData.value.lung_family_history)
          ? formData.value.lung_family_history.join(',')
          : formData.value.lung_family_history,
        lung_family_history_other: formData.value.lung_family_history_other,
        lung_medication_history: Array.isArray(formData.value.lung_medication_history)
          ? formData.value.lung_medication_history.join(',')
          : formData.value.lung_medication_history,
        lung_medication_other: formData.value.lung_medication_other,
  ...
  <div class="record-form-view">
    ...
    <header class="page-header-compact">
      <div class="header-left">
        ...
        <h2 class="page-title">乳腺+肺双结节档案</h2>
      </div>
      ...
      <form @submit.prevent="submitForm">
        ...
        <div class="dual-nodule-container">
          ...
          <section class="form-section imaging-section">
            ...
            <BreastFormSection v-model="formData" :showCommonHistory="true" :usePerOrganHistory="true" />
          </section>

          <!-- 肺部影像学特征 -->
          <section class="form-section imaging-section-lung">
            <div class="section-header">
              <div class="section-icon"></div>
              <h3 class="section-title">（三）肺部影像学特征</h3>
            </div>

            <!-- 使用 LungFormSection 组件 -->
            <LungFormSection v-model="formData" :usePrefixedFields="true" :showCommonHistory="true" />
          </section>
        </div>
