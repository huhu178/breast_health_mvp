<template>
  <section class="login-page" :style="themeVars">
    <div class="login-brand">
      <div class="login-brand-mark">{{ scenario.loginLabel.slice(0, 1) }}</div>
      <div>
        <div class="login-brand-name">{{ scenario.brandTitle }}</div>
        <div class="login-brand-sub">{{ scenario.orgName }}</div>
      </div>
    </div>
...
const themeVars = computed(() => ({
  '--login-primary': scenario.value.theme?.primary || '#155eef',
  '--login-soft': scenario.value.theme?.soft || '#eef5ff'
}))
...
.login-page::after{
  ...
  background:linear-gradient(to right, rgba(15,23,42,.1) 0, rgba(15,23,42,.05) 34%, rgba(248,251,255,.72) 58%, #f8fbff 88%);
  pointer-events:none;
  z-index:0;
}
.login-brand{position:absolute;top:36px;left:44px;display:flex;align-items:center;gap:10px;z-index:2}
.login-brand-mark{width:38px;height:38px;border-radius:10px;background:var(--login-primary,#155eef);color:#fff;display:grid;place-items:center;font-weight:900;font-size:15px;flex-shrink:0}
.login-brand-name{font-size:18px;font-weight:700;color:#fff;text-shadow:0 1px 6px rgba(0,0,0,.35);letter-spacing:.3px}
.login-brand-sub{margin-top:3px;color:rgba(255,255,255,.82);font-size:12px;font-weight:600;text-shadow:0 1px 5px rgba(0,0,0,.35)}

.login-panel-wrap{position:relative;z-index:1;display:flex;align-items:center;justify-content:center}
.login-panel{width:420px;background:rgba(255,255,255,.97);border:1px solid #dce8f5;border-radius:12px;padding:30px 32px;box-shadow:0 8px 32px rgba(15,23,42,.10),0 1px 3px rgba(15,23,42,.06)}
.login-title{font-size:24px;font-weight:900;color:#111827;margin-bottom:8px}
.login-sub{color:#64748b;margin-bottom:20px}
...
.login-submit{width:100%;height:42px;border:0;border-radius:6px;background:var(--login-primary,#155eef);color:#fff;font-weight:850;margin-top:8px;cursor:pointer}
.login-submit:hover{filter:brightness(.94)}
...
.login-hint button{border:0;background:transparent;color:var(--login-primary,#155eef);padding:0;cursor:pointer;font-weight:700}
