...
def get_all_patients():
    ...
    for patient in b_patients:
        ...
        # 取最新报告的风险等级
        latest_report = BReport.query.filter_by(patient_id=patient.id).order_by(BReport.created_at.desc()).first()
        patient_data['risk_level'] = latest_report.risk_level if latest_report else None
...
def generate_patient_report(patient_id):
    ...
    try:
        ...
        traceback.print_exc()
        return Response.error(f'报告生成失败: {str(e)}')
